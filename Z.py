...
#os.system(f"ffmpeg -http_persistent 0 -re -i '{pr}' -max_muxing_queue_size 9999 -vf \"[1:v]format=yuv420p, movie=bunny4.png[watermark];scale=iw/1.7:ih/1.7[watermark_scaled];[in][watermark_scaled]overlay=main_w-overlay_w-5:main_h-overlay_h-5\" -c:a copy -g 48 -preset ultrafast -tune zerolatency -f flv rtmp://a.rtmp.youtube.com/live2/zvmf-1yjp-jzek-01pw-b4js")
...
#short -ß
os.system(f"ffmpeg -http_persistent 0 -re -ss 05:59:00 -i '{pr}' -max_muxing_queue_size 9999 -threads 4 -vf \"format=yuv420p,drawtext=fontfile=_.ttf:text='F B':fontcolor=white:fontsize=130:line_spacing=32:bordercolor=black:borderw=10:box=0:boxcolor=black@0.1:boxborderw=80:x=w-tw-20:y=h-th-20,transpose=1\" -c:v libx264 -g 48 -b:v 9000k -c:a aac -preset ultrafast -tune zerolatency -f flv rtmp://a.rtmp.youtube.com/live2/q97f-38zg-czwr-qwah-b2wy")
